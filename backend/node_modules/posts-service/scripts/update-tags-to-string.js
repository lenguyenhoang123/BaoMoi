...
// Cập nhật tất cả bài viết để đổi cấu trúc tags thành chuỗi
async function updateTagsToString() {
  ...
  try {
    ...
    for (const row of result.rows) {
      // Nếu tags là mảng, chuyển thành chuỗi cách nhau bởi dấu phẩy
      let tagsString = '';
      if (Array.isArray(row.tags)) {
        tagsString = row.tags.join(', ');
      } else if (typeof row.tags === 'string') {
        // Nếu đã là chuỗi rồi thì giữ nguyên
        tagsString = row.tags;
      }
    ...
    console.log('\n🎉 Đã cập nhật tất cả tags thành chuỗi thành công!');
  } catch (error) {
...
// Chạy hàm cập nhật
console.log('🔄 Đang cập nhật tags thành chuỗi...\n');
updateTagsToString();
