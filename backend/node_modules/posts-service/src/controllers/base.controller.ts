...
export default class BaseController {
  ...
  protected sendForbidden(
    res: Response,
    message: string = 'Từ chối truy cập'
  ): void {
  ...
  protected getPaginationParams(req: Request): { page: number; limit: number } {
    ...
    const limit = Math.min(parseInt(req.query.limit as string, 10) || 10, 100); // Tối đa 100 bản ghi mỗi trang
    return { page, limit };
  }
}
