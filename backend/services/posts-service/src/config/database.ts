import pg, { Pool, types, QueryResult, QueryResultRow } from 'pg';

// Cấu hình kiểu dữ liệu số
types.setTypeParser(1700, parseFloat); // Xử lý kiểu dữ liệu numeric
  ...
  async getClient() {
    ...
    try {
      ...
      return {
        ...
        release: (err?: Error | boolean) => client.release(err),
      };
    } catch (error: any) {
