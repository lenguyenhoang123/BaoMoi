    ...
    `);
      ...
      `);
            ...
            for (const row of categoriesResult.rows) {
                ...
                const checkCategory = await client.query('SELECT id FROM categories WHERE name = $1', [categoryName]);
                let categoryId;
                if (checkCategory.rows.length > 0) {
                    ...
                    const newCategory = await client.query('INSERT INTO categories (name, slug) VALUES ($1, $2) RETURNING id', [categoryName, categoryName.toLowerCase().replace(/\s+/g, '-')]);
                    categoryId = newCategory.rows[0].id;
                    console.log(`Đã tạo mới category: ${categoryName} (ID: ${categoryId})`);
                }
                // Lưu vào bảng ánh xạ
                await client.query('INSERT INTO category_mapping (category_name, category_id) VALUES ($1, $2) ' +
                    'ON CONFLICT (category_name) DO UPDATE SET category_id = EXCLUDED.category_id', [categoryName, categoryId]);
            }
